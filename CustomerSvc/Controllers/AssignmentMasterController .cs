...
namespace CustomerSetup.Controllers
{
    ...
    public class AssignmentMasterController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ActionResult<ServiceResponse<List<GetAssignmentMasterDto>>>> Get()
        {
            return Ok(await _ctrlSvc.GetAllAssignments());
        }
        ...
        public async Task<ActionResult<ServiceResponse<List<GetAssignmentMasterDto>>>> DeleteAssignment(int id)
        {
            var response = await _ctrlSvc.DeleteAssignment(id);
            if (response.Data == null)
            {
                return NotFound(response);
            }

...
namespace GRCServices.Controllers
{
    ...
    public class ActivityMasterController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ActionResult<ServiceResponse<List<GetActivityMasterDto>>>> Get()
        {
            return Ok(await _ctrlSvc.GetAllActivites());
        }
        ...
        public async Task<ActionResult<ServiceResponse<List<GetActivityMasterDto>>>> DeleteRole(int id)
        {
            var response = await _ctrlSvc.DeleteActivity(id);
            if (response.Data == null)
            {
                return NotFound(response);
            }

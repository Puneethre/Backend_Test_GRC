...
namespace GRCServices.Controllers
{
    ...
    public class RoleMasterController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<RoleTypeLookup>>> Getroletypess()
        {
            try
            {
                var cutomeslist = _context.RoleTypes.ToList();

                List<RoleTypeLookup> roletypes = new List<RoleTypeLookup>();
                foreach (var rec in cutomeslist)
                {
                    RoleTypeLookup rtl = new RoleTypeLookup();
                    rtl.RoleTypeId = rec.RoleTypeId;
                    rtl.RoleTypeDescription = rec.RoleTypeDescription;
                    roletypes.Add(rtl);
                }
        ...
        [HttpGet("roleslookup")]
        public async Task<ActionResult<List<RoleLookUp>>> Getrolelookup()
        {
            try
            {
                var cutomeslist = _context.ClientRoleMasters.Where(r => r.RoleTypeId != 0).ToList();
        ...
        public async Task<ActionResult<ServiceResponse<List<GetRoleMasterDto>>>> DeleteRole(int id)
        {
            var response = await _ctrlSvc.DeleteRole(id);
            if (response.Data == null)
            {
                return NotFound(response);
            }
